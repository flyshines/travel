package qingning.shop.server.configuration;
...
@Configuration
@EnableWebMvc
@ComponentScan({"qingning.shop.server.controller","qingning.server.advice"})
...
public class SystemConfiguration extends WebMvcConfigurerAdapter {
	...
	public ServiceManger getServiceManger() throws Exception{
		...
		serviceManger.initServer(new String[]{"classpath:sharing-shop-server.xml"});
	...
	public ClassPathXmlApplicationContext getApplicationContext(){
		if(context==null){
			context=new ClassPathXmlApplicationContext(new String[] {"sharing-shop-rpc-server.xml"});
			context.start();
		}
		return context;
	}
