package qingning.user.common.server.configuration;
...
@Configuration
@EnableWebMvc
@ComponentScan({"qingning.user.common.server.controller","qingning.server.advice"})
...
public class SystemConfiguration extends WebMvcConfigurerAdapter {
	...
	public ServiceManger getServiceManger() throws Exception{
		...
		serviceManger.initServer(new String[]{"classpath:sharing-user-common-server.xml"});
	...
	public ClassPathXmlApplicationContext getApplicationContext(){
		if(context==null){
			context=new ClassPathXmlApplicationContext(new String[] {"sharing-user-common-rpc-server.xml"});
			context.start();
		}
		return context;
	}
